 ...
 * class, do not.
 *  Map接口提供3种集合视图：
 *      一串key的集，value的集合和一串键值对的映射的集
 *  map的顺序被map的迭代器视图返回的元素定义。
 *  一些Map的实现是有序的，比如TreeMap，其他的，比如HashMap是无序的。
 *
 ...
 *
 *  注意：必须小心谨慎当一个可变对象作为map的key。
 *
 *  he behavior of a map is not specified if the value of an object is
 *  * changed in a manner that affects {@code equals} comparisons while the
 *  * object is a key in the map.
 *  map的行为是不明确的，如果一个对象是
 *  baidu: 当对象是映射中的键时，如果对象的值以影响equals比较的方式更改，则不指定映射的行为。
 *
 *  一个特点的例子是禁止一个map把自身作为键，但允许作为值，
 *  要极度小心：equals和hashCode方法在这种map中没有很好的定义了。
 *
 *
 * <p>
 *      所有通用的map实现类需要提供两个标准的构造器：一个创建空map的空（无参）构造器和有一个Map类型参数的构造器，
 *  创建一个和参数有相同键值对的map。事实上后一个构造器允许用户用来复制任何map，生成所需类的等价map。
 *  没有办法强制这个建议（因为接口不能包含构造器），但是JDK所有通用map的实现都符合。
 ...
 *
 * <p>
 *     此接口中包含的“破坏性”方法，即修改其操作的映射的方法，被指定为在该映射不支持该操作时
 *     引发UnsupportedOperationException。
 *     如果是这种情况，如果调用对映射没有影响，则这些方法可以（但不是必需）抛出UnsupportedOperationException。
 *     例如，如果要“叠加”映射的映射为空，则对不可修改的映射调用putAll（Map）方法可能（但不是必需）引发异常。
 *
 ...
 *
 * <p>
 *     一些map的实现限制它们包含的键值。比如，一些实现允许空的键和值，还有一些对键的类型有限制。
 *     Some map implementations have restrictions on the keys and values they
 * may contain.  For example, some implementations prohibit null keys and
 * values, and some have restrictions on the types of their keys.  Attempting
 * to insert an ineligible（不符合资格的） key or value throws an unchecked exception,
 ...
 * 许多集合框架接口的方法都根据equals方法定义
 *
 * For example, the specification for the {@link #containsKey(Object)
 ...
 *  比如containsKey(Object key)的方法说说明：
 *      当且仅当map包含的key满足(key==null ? k==null : key.equals(k))的条件时，返回true
 *
 * This specification should <i>not</i> be construed to imply that invoking {@code Map.containsKey}
 ...
 *
 * <p>
 *     一些递归遍历map操作可能会失败，但map直接或间接包含它自身的自引用的实例除外。
 *     包括clone()、 equals()、 equals()、toString()方法。
 ...
 * Implementations may optionally handle the self-referential scenario（设想;方案;预测;(电影或戏剧的)剧情梗概）,
 * however most current implementations do not do so.
 *
 * <h2><a id="unmodifiable">Unmodifiable Maps</a></h2> 不可变的Map
 * <p>The {@link Map#of() Map.of},
 * {@link Map#ofEntries(Entry...) Map.ofEntries}, and
 * {@link Map#copyOf Map.copyOf}
 * 静态工厂方法提供一个方便的创建不可变map的方式。
 * static factory methods provide a convenient way to create unmodifiable maps.
 *
 * 被这些方法创建的Map实例有以下特点：
 * The {@code Map} instances created by these methods have the following characteristics（特点）:
 *
 * <ul>
 * <li>They are <a href="Collection.html#unmodifiable"><i>unmodifiable</i></a>.
 ...
 *  对Map调用任何更改器方法或抛出UnsupportedOperationException
 *
 * However, if the contained keys or values are themselves mutable, this may cause the
 * Map to behave inconsistently or its contents to appear to change.
 *  但是，如果包含的键或值本身是可变的，则这可能会导致Map的行为不一致或其内容似乎发生更改。
 *
 * <li>They disallow {@code null} keys and values.
 * 不允许空的键值
 * Attempts to create them with {@code null} keys or values result in {@code NullPointerException}.
 * 试图用空的键值创建它会导致NullPointerException
 *
 * <li>They are serializable if all keys and values are serializable.
 * <li>They reject duplicate keys at creation time.
 * Duplicate keys passed to a static factory method result in {@code IllegalArgumentException}.
...
/***
 * Map设计思想？
 *  当<key,value>的值为<"key",null>和不存在键"key"时，get("key")返回值都是null，
 *  说明当值被删除（为null时），键名没有意义
 * @param <K>
 * @param <V>
 */
public interface Map<K, V> {
    ...
    default V getOrDefault(Object key, V defaultValue) {
        V v;
        return (((v = get(key)) != null) || containsKey(key)) // containsKey没有实现
            ? v
            : defaultValue;
    }
     ...
     * @param key
     * @param value
     * @param remappingFunction  BiFunction 两个参数带返回值的函数
     * @return
     */
    default V merge(K key, V value,
        ...
        V newValue = (oldValue == null) ? value :
                   remappingFunction.apply(oldValue, value);
        if (newValue == null) {  // 新值为null时直接删除对应的键，getKey(key)的结果一样
            remove(key);
        } else {
            put(key, newValue);
        }
