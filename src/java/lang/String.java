 ...
 *  String类代表字符串，在Java程序中所有的字符串字面量都是一个String类的实例
 *
 * <p>
 * Strings are constant; their values cannot be changed after they
 * are created. String buffers support mutable strings.
 *  字符串是常量，它们被创建后的值不能改变。StringBuffer是可变的字符串。
 *
...
public final class String
     ...
     */
    /***
     * 从jdk1.9开始该类的底层不使用char[]来存储数据，而是改成 byte[]加上编码标记coder，
     * 从而节约了一些空间
     *
     */
    @Stable
    private final byte[] value; // 存储字符
    ...
    public String(byte bytes[], int offset, int length) {
        checkBoundsOffCount(offset, length, bytes.length);
        StringCoding.Result ret = StringCoding.decode(bytes, offset, length); // 字节数组解码成
        this.value = ret.value;
        this.coder = ret.coder;
    }
     ...
     * @since  1.5
     */
    public static String format(Locale l, String format, java.lang.Object... args) {
    ...
    String(char[] value, int off, int len, Void sig) {
        ...
        this.coder = UTF16;
        this.value = StringUTF16.toBytes(value, off, len); // 把字符数组转为字节数组
    }
